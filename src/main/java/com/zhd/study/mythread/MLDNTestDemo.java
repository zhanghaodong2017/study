...
public class MLDNTestDemo {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 设置延迟队列
        DelayQueue<Memberss> queue = new DelayQueue<Memberss>();
        queue.add(new Memberss("张三", 3, TimeUnit.SECONDS));
        queue.add(new Memberss("李四", 5, TimeUnit.SECONDS));
        // 如果聚会还有人在呢
        while (!queue.isEmpty()) {
            // 从里面取出数据内容
            Delayed dyd = (Delayed) queue.poll();
...
class Memberss implements Delayed {
    // 聚会人员的名单
    private String name;
    // 失效时间，人员离开的时间，毫秒单位
    private long expire;
     ...
     * @param unit  时间处理单位
     */
    public Memberss(String name, long delay, TimeUnit unit) {
        this.name = name;
        // 保存延迟的时间
